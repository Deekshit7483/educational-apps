...
public class ShareFileActivity extends AppCompatActivity {
	...
	public final int REQ_CD_FIL = 101;
	...
	private StorageReference files = _firebase_storage.getReference("files");
	...
	private void initialize(Bundle _savedInstanceState) {
		...
		button1.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View _view) {
				startActivityForResult(fil, REQ_CD_FIL);
			}
		});
		...
		button2.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View _view) {
				if (filename.equals("")) {
					SketchwareUtil.showMessage(getApplicationContext(), "Please pick pdf file");
				}
				else {
					if (edittext1.getText().toString().equals("")) {
						SketchwareUtil.showMessage(getApplicationContext(), "Enter pdf name");
					}
					else {
						files.child(Uri.parse(Uri.parse(filename).getLastPathSegment()).getLastPathSegment()).putFile(Uri.fromFile(new File(filename))).addOnFailureListener(_files_failure_listener).addOnProgressListener(_files_upload_progress_listener).continueWithTask(new Continuation<UploadTask.TaskSnapshot, Task<Uri>>() {
							@Override
							public Task<Uri> then(Task<UploadTask.TaskSnapshot> task) throws Exception {
								return files.child(Uri.parse(Uri.parse(filename).getLastPathSegment()).getLastPathSegment()).getDownloadUrl();
							}}).addOnCompleteListener(_files_upload_success_listener);
					}
				}
			}
		});
		...
		_files_upload_success_listener = new OnCompleteListener<Uri>() {
			...
			public void onComplete(Task<Uri> _param1) {
				...
				keyy = file.push().getKey();
				map = new HashMap<>();
				map.put("pdf name", edittext1.getText().toString().concat(".pdf"));
				map.put("pdf path", _downloadUrl);
				map.put("uid", FirebaseAuth.getInstance().getCurrentUser().getUid());
				map.put("key", keyy);
				file.child(keyy).updateChildren(map);
				map.clear();
